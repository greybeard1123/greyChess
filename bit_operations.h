...
unsigned int bitCnt(U64 bitmap)
{
	...
	// BIT HACKS by Sean Eron Anderson, see http://graphics.stanford.edu/~seander/bithacks.html
...
void displayBitmap(BitMap in)
{
	...
	cout << endl << "  firstOne = " << firstOne(in) <<", bitCnt = " << bitCnt(in) << endl;
	cout << endl << endl;

	if (board.is_viewRotated())
	{
