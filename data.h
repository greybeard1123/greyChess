...
void dataInit()
{
    ...
    //Setting only one bit in BITSET
	BITSET[0] = 0x1;
	for (i = 1; i < 64; i++)
	{
	...
	//Here file is from 1..8 and rank from 1..8 (not starting from 0)
	...
	//initialize the rank, file and diagonal 6-bit masking bitmaps to get the occupancy states
	...
	for (file = 1; file < 9; file++)
	{
		...
		{
			//initialize the 6-bit rank mask
			...
			//initialize the 6-bit file mask
			...
			//initialize file magic multiplication numbers
			FILEMAGIC[BOARDINDEX[file][rank]] = _FILEMAGICS[file - 1];

		}
	}

	// initialize CHARBITSET: 8 chars, each with only 1 bit set.
	CHARBITSET[0] = 1;
	...
	for (square = 0; square < 64; square++)
	{
		...
		//Masks for castling : 0 for white and 1 for black
		...
		//Initialize the evaluation data and bitmaps
		BLACK_SQUARES = 0;
		for (i = 0; i < 64; i++)
		{
		...
		//DISTANCE array, distance is measured as max(rank,file)-difference
		for (i = 0; i < 64; i++)
		{
			for (square = 0; square < 64; square++)
			{
		...
		//Initialize the mirrored data for white
		for (square = 0; square < 64; square++)
		{
